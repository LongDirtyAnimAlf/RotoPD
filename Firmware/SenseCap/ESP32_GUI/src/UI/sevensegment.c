...
byte WordToTxtThousands(uint32_t value, byte displaysize, byte divider, char* destination)
{
  static word dividers[] = { 9U, 99U, 999U, 9999U};  
  int8_t dot;
  uint32_t tmp;
  int8_t runner;
  char * buffer;
  ...
  // Fill buffer with zero and calulate dot position
  uint8_t ndx = displaysize;
  while (ndx)
  {
    ndx--;
    buffer[ndx]  = '0';
    if (tmp>dividers[divider]) dot++;
    tmp /= 10; 
  }
...
void DWordToTxt(uint32_t value, byte displaysize, char* destination)
{
  uint32_t tmp;
  int8_t runner;
  char * buffer;
...
void SetDisplaymV(lv_obj_t * target, unsigned long value)
{
  if (target != NULL)
  {
    ...
    if ((digits_back != NULL) && (digits_front != NULL))  
    {
      ...
      byte dp = WordToTxtThousands(value,digitcount,3,tmpArray);
      lv_label_set_text(digits_front, tmpArray); 
...
void SetDisplayV(lv_obj_t * target, unsigned long value)
{
  if (target != NULL)
  {
    lv_obj_t * digits_back = lv_obj_get_child(target, 0);
    ...
    if ((digits_back != NULL) && (digits_front != NULL))  
    {
      ...
      byte dp = WordToTxtThousands(value,digitcount,1,tmpArray);
      lv_label_set_text(digits_front, tmpArray); 
