...
public:
    ...
    static void         decodePDONew(uint8_t idx, PDO_DATA_T &raw, AP33772S_PDO &dec);

    // ── Voltage requests ────────────────────────────────────────────────────
    // All mV and mA parameters are in those units

    void    setMaxPDO(uint8_t pdoIndex);
    ...
    bool clearConfig(uint8_t flag);
    bool setConfig(uint8_t flag);
