...
{
    // Check wire by reading a RO register
    // If succeeds then continue;
    int16_t s = readReg8(CMD_OPMODE);
    ...
    if (!writeReg8(CMD_CONFIG, 0)) return AP33772S_ERR_I2C;
    if (!writeReg8(CMD_MASK, MASK_ALL)) return AP33772S_ERR_I2C;

    //delay(100);
    /*
    After boot up (after 100ms) expect:
    NEWPDO      = 1
    READY       = 1
    STARTED     = 1
...
// ── pollI2CReady() ────────────────────────────────────────────────────────────
bool AP33772S::pollI2CReady(uint32_t timeoutMs)
{
    uint32_t dl = millis() + timeoutMs;
    while (millis() < dl) {
        uint8_t s = getStatus();
        if (s & STATUS_READY) return true;
        delay(50);
    }
    return false;
...
void AP33772S::decodePDONew(uint8_t idx, PDO_DATA_T &raw, AP33772S_PDO &dec)
{
    dec.index    = idx + 1;
...
void AP33772S::_decodePDO(uint8_t idx)
{
    ...
    decodePDONew(idx, raw, dec);
}

int8_t  AP33772S::getValidPDOCount()
{
...
void AP33772S::setMaxPDO(uint8_t pdoIndex)
{
    _sendRDO(pdoIndex, 0x0F, 0xFF);
}
...
bool AP33772S::clearConfig(uint8_t flag)
{
  // First read in what is currently in the config
  uint8_t r = (uint8_t)readReg8(CMD_CONFIG);
  r &= ~flag;   // reset the bit
  return writeReg8(CMD_CONFIG, r);
}

bool AP33772S::setConfig(uint8_t flag)
{
  // First read in what is currently in the config
  uint8_t r = (uint8_t)readReg8(CMD_CONFIG);
  r |= flag;   // set the bit
