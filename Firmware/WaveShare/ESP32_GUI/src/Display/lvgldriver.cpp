...
// https://github.com/moononournation/Arduino_GFX/issues/638
// https://github.com/moononournation/Arduino_GFX/issues/637

#define BYTE_PER_PIXEL (LV_COLOR_FORMAT_GET_SIZE(LV_COLOR_FORMAT_RGB565)) /*will be 2 for RGB565 */

#define LVGL_DRAW_BUF_LINES  20 // number of display lines in each draw buffer in partial mode
...
void my_disp_flush_direct_tft(lv_disp_drv_t * disp, const lv_area_t* area, lv_color_t * px_map)
{
  uint32_t w = (area->x2 - area->x1 + 1);
  uint32_t h = (area->y2 - area->y1 + 1);
  TFT.startWrite();
  TFT.setAddrWindow(area->x1, area->y1, w, h);
  TFT.pushColors(&px_map->full, w * h, true);
  TFT.endWrite();
...
void IRAM_ATTR my_disp_flush_direct(lv_display_t * disp, const lv_area_t * area, uint8_t * px_map)
{
    #ifndef DIRECT_GFX
    ...
    uint32_t fb_stride = lv_draw_buf_width_to_stride(disp_hres, cf);
    ...
    {
      // Direct copy !!
      if (src_stride == fb_stride)
      {
        int32_t src_h = lv_area_get_height(area);
        lv_memcpy(fb_start, px_map, (src_stride * src_h));
      }
      else
      {
        int32_t y;
...
void lv_screen_init(void * gfx, word W, word H)
{
  ...
  #else
  static uint8_t * sbuf2_1 = (uint8_t *)heap_caps_aligned_alloc(4, W * H * BYTE_PER_PIXEL, MALLOC_CAP_8BIT | MALLOC_CAP_SPIRAM);
  //static uint8_t * sbuf2_2 = (uint8_t *)heap_caps_aligned_alloc(4, W * H * BYTE_PER_PIXEL, MALLOC_CAP_8BIT | MALLOC_CAP_SPIRAM);
  static uint8_t * sbuf2_2 = NULL;
  #endif
...
void my_disp_flush_simple(lv_display_t *disp, const lv_area_t *area, uint8_t * px_map)
{
  ...
  gfxdisplay->draw16bitBeRGBBitmap(area->x1, area->y1, (uint16_t *)px_map, w, h);    
#else
  //gfxdisplay->draw16bitRGBBitmap(area->x1, area->y1, buf16, w, h);
  gfxdisplay->draw16bitRGBBitmap(area->x1, area->y1, (uint16_t *)px_map, w, h);    
#endif
...
void lv_screen_init(void * gfx, word W, word H)
{
  ...
  //static uint8_t * sbuf1_1 = (uint8_t *)heap_caps_malloc(draw_buffer_sz, MALLOC_CAP_DMA);
  static uint8_t * sbuf1_1 = (uint8_t *)heap_caps_aligned_alloc(4, draw_buffer_sz, MALLOC_CAP_SPIRAM);
  ...
  //static uint8_t * sbuf1_2 = (uint8_t *)heap_caps_malloc(draw_buffer_sz, MALLOC_CAP_DMA);
  //static uint8_t * sbuf1_2 = (uint8_t *)heap_caps_malloc( draw_buffer_sz, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);  
  static uint8_t * sbuf1_2 = (uint8_t *)heap_caps_aligned_alloc(4, draw_buffer_sz, MALLOC_CAP_SPIRAM);
